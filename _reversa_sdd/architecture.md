...
**fullstack-profile** is a statically exported Next.js 14 single-page portfolio. There is no server, no database, no API, and no authentication. All content is baked at build time and served as static files via GitHub Pages.

| Property | Value |
|----------|-------|
| Architecture style | Static Site / JAMstack |
| Deployment target | GitHub Pages (free tier) |
| Runtime | Browser only — no server |
| Data layer | JSON files bundled at build time |
...
│        gtoshinakano.github.io/fullstack-profile/        │
└────────────────────────┬────────────────────────────────┘
                         │ HTTPS request
                         ▼
┌─────────────────────────────────────────────────────────┐
...
│  │  ┌──────────┐  ┌──────────┐  ┌────────────────┐ │  │
│  │  │ Hero     │  │ HeroDark │  │ MainContent    │ │  │
│  │  │ Section  │  │ (tabs)   │  │ (3-3-3 article)│ │  │
│  │  └──────────┘  └──────────┘  └────────────────┘ │  │
│  │  ┌──────────────────────────────────────────────┐ │  │
│  │  │ Layout: Public + ChangeLanguage widget       │ │  │
│  │  └──────────────────────────────────────────────┘ │  │
│  └──────────────────────────────────────────────────┘  │
│                                                         │
